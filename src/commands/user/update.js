...
update.action(async(newUserID) => {
    try {
        ...
        for(const problemNumber in context.data.problems.challenging){
            ...
            if(fs.existsSync(problem.getUserSolutionPath(beforeID)))    problem.changeFileName(problem.getUserSolutionPath(beforeID),problem.getUserSolutionPath(newUserID));
        }
